  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center px-4 transition-colors ${dark ? 'bg-dark-bg' : 'bg-gray-50'}`}>
      ...
      <div className="mb-10 text-center">
        ...
        <p className={`text-sm mt-1.5 ${dark ? 'text-gray-400' : 'text-gray-500'}`}>by El Mayorista — Mission Control</p>
      </div>
      ...
      <div className="w-full max-w-md">
        <div className={`rounded-2xl shadow-sm border p-8 ${dark ? 'bg-dark-card border-dark-border' : 'bg-white border-gray-200'}`}>
          <h2 className={`text-lg font-semibold mb-6 ${dark ? 'text-white' : 'text-gray-900'}`}>Iniciar Sesion</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className={`block text-sm font-medium mb-1.5 ${dark ? 'text-gray-300' : 'text-gray-700'}`}>Contrasena</label>
        ...
        <p className={`text-center text-xs mt-6 ${dark ? 'text-gray-600' : 'text-gray-400'}`}>
          El Mayorista &copy; {new Date().getFullYear()} — OmniVident v1.0
        </p>
      </div>
    </div>
  );
